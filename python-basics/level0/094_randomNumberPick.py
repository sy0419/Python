...
# 제한사항) 1 ≤ arr의 길이 ≤ 100,000 , 0 ≤ arr의 원소 ≤ 100,000 , 1 ≤ k ≤ 1,000

def solution(arr, k):
    newArr = []
    for element in range(len(arr)):
        if arr[element] not in newArr:
            newArr.append(arr[element])
        if len(newArr) == k:
            break
    if len(newArr) < k:
        newArr.extend([-1] * (k - len(newArr)))
    return newArr
...
print("----------------")
print(solution2([0, 1, 1, 2, 2, 3], 3))
print(solution2([0, 1, 1, 1, 1], 4))